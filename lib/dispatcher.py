	...
	def receive(self):
		while not self.slicer.file_done:
			try:
				...
				piece = lib.slicer.data_piece(self.slicer.file_path, piece_id, piece_data, piece_size, piece_MD5)
				...
				for ids in self.slicer.get_missed_parts():
					if ids != piece_id:
						url = '/lost/{}'.format(ids)
						self.tunnel.request('GET', url)
			except http.client.BadStatusLine as BadStatusLine:
				for i in BadStatusLine.line:
					print('%#x'%ord(i), end = " ")
